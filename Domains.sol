...
contract Domains is ERC721URIStorage {
    ...
    mapping(string => address) domainsToOwner;
    ...
    event NewDomainRegistered(string, address);

    constructor(string memory _tld) payable ERC721("DXB Name Service", "DXB") {
        tld = _tld;
    ...
    function register(string calldata name) public payable {
        if (domainsToOwner[name] != address(0)) revert AlreadyRegistered();
        ...
        domainsToOwner[name] = msg.sender;

        _tokenIds.increment();
        names[newRecordId] = name;
    }

    function getOwnerAddress(string calldata name)
        external
        view
        returns (address)
    {
        return domainsToOwner[name];
    }

    function setRecord(string calldata name, string calldata record) public {
        if (msg.sender != domainsToOwner[name]) revert Unauthorized();
        records[name] = record;
    }
